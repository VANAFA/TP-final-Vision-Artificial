...
def _roi_mask(edges, row_top=0.60, row_bot=0.90):
    ...
    vertices = np.array([[
        [int(w * 0.18), int(h * row_bot)],
        [int(w * 0.44), int(h * row_top)],
        [int(w * 0.56), int(h * row_top)],
        [int(w * 0.82), int(h * row_bot)],
    ]], dtype=np.int32)
...
def _avg_lane(lines, img_width):
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            ...
            if slope < 0 and x_mid < img_width * 0.65:
                left_si.append((slope, intercept))
                left_w.append(length)
            elif slope > 0 and x_mid > img_width * 0.35:
...
def detect_lane_lines(img_bgr, row_top=0.50, row_bot=0.75):
    ...
    # Reject if lines crossed at road level (detection is inverted)
